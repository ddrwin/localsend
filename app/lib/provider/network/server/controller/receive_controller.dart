...
/// Writes debug log to a file so user can check what happened.
void _debugFileLog(String message) {
  _logger.info(message);
  try {
    final logFile = File('C:\\localsend\\localsend_autocopy.log');
    logFile.writeAsStringSync('${DateTime.now().toIso8601String()} $message\n', mode: FileMode.append);
  } catch (e) {
    _logger.severe('Failed to write debug log: $e');
  }
}
...
class ReceiveController {
  ...
  Future<void> _prepareUploadHandler({
    ...
    _debugFileLog('=== _prepareUploadHandler entered ===');
    ...
    final bool autoAcceptMessage = message != null && settings.autoCopyText;
    final msgPreview = message != null ? message.length > 50 ? '${message.substring(0, 50)}...' : message : 'null';
    _debugFileLog('_prepareUploadHandler: message=$msgPreview, autoAcceptMessage=$autoAcceptMessage, autoCopyText=${settings.autoCopyText}');
    ...
    if (quickSave) {
      ...
      if (checkPlatformHasTray() && (await windowManager.isMinimized() || !(await windowManager.isVisible()) || !(await windowManager.isFocused()))) {
        if (!autoAcceptMessage) {
          _debugFileLog('showFromTray in prepare handler: window needs showing, calling showFromTray');
          await showFromTray();
        } else {
          _debugFileLog('showFromTray in prepare handler: skipped (autoAcceptMessage=true)');
        }
      }
      ...
      if (message != null) {
        ...
        if (settings.autoCopyText) {
          ...
          if (settings.autoSendOnPaste) {
            ...
            await simulateEnter();
            _debugFileLog('prepareUploadHandler: simulateEnter returned');
          }
      ...
      if (!autoAcceptMessage) {
        _debugFileLog('autoAcceptMessage=false: showing ReceivePage');
        ...
        _debugFileLog('autoAcceptMessage=true: skipping ReceivePage push');
      }
  ...
  Future<void> _showHandler({
    ...
    if (senderToken == showToken && checkPlatformIsDesktop()) {
      final autoCopyText = server.ref.read(settingsProvider).autoCopyText;
      _debugFileLog('_showHandler: autoCopyText=$autoCopyText, showing window=${!autoCopyText}');
      if (!autoCopyText) {
